  ...
  return (
    ...
    <SafeAreaView style={{paddingTop:Platform.OS==='android'?40:0,flex:1,backgroundColor:'white'}}>
      <ScrollView>
        ...
        <Pressable onPress={()=>setModalVisible(!modalVisible)} style={styles.locationView}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {list.map((item,index)=>(<Pressable key={index} style={{margin:10,justifyContent:'center',alignItems:'center'}} >
        ...
        <SliderBox images={images} autoPlay circleLoop inactiveColor='#98A4AE' dotColor={'#13274F'} imageComponentStyle={{width:'100%'}} />
      ...
      <Text style={{padding:10,fontSize:18,fontWeight:'bold'}}>Today's Deals</Text>
        ...
        {offers.map((item,index)=>(
          <Pressable style={{marginVertical:10,alignItems:'center',justifyContent:'center'}} key={index} onPress={()=>navigation.navigate("Info",{
            id:item.id,
            title:item.title,
            price:item?.price,
            carouselImages:item.carouselImages,
        ...
        <Text style={{height:1,borderColor:'#D0D0D0',borderWidth:2,marginTop:15}} />
         ...
         <View style={{marginTop:20,marginHorizontal:10,width:'45%',marginBottom:open?50:15}} >  
          <DropDownPicker 
      ...
      <ModalContent style={{width:'100%',height:400}}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          ...
          {addresses.map((item,index)=>(
            <Pressable onPress={()=>{setSelectedAddress(item);setModalVisible(false);}} key={index} style={{width:150,height:140,borderColor:'#D0D0D0',borderWidth:1,padding:10,justifyContent:'center',alignItems:'center',gap:3,marginRight:15,backgroundColor:selectedAddress===item?'#FBCEB1':'white'}} >
             <View style={{flexDirection:'row',alignItems:'center',gap:3}} >
                    <Text style={{fontSize:13,fontWeight:'bold'}} >{item.name}</Text>
                    <Entypo name="location-pin" size={24} color="red" />
                  </View>
                  <Text numberOfLines={1} style={{width:135,fontSize:13,textAlign:'center'}}>{item?.houseNo}, {item?.landmark}</Text>
                  <Text numberOfLines={1} style={{width:135,fontSize:13,textAlign:'center'}}>{item?.street}</Text>
